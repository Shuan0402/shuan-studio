  ...
  return (
    <div className="w-1/2 h-full p-15 flex flex-col overflow-y-auto bg-white/40">
      ...
      <div className="space-y-6 mt-8">
        {/* Technologies */}
        <div>
          <h4 className="text-stone-400 text-[10px] font-black mb-3 uppercase tracking-[0.2em]">
            Technologies
          </h4>
          <div className="flex flex-wrap gap-2">
            {project.technologies.map((tech, i) => (
              <span 
                key={i} 
                className="bg-blue-50 text-blue-600 border border-blue-100 text-[11px] font-bold px-3 py-1 rounded-md"
              >
                {tech}
              </span>
            ))}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag, i) => (
              <span 
                key={i} 
                className="bg-stone-100 text-stone-500 border border-stone-200 text-[11px] font-bold px-3 py-1 rounded-md"
              >
                {tag}
              </span>
            ))}
      ...
      {/* 外部連結按鈕區 */}
      <div className="flex items-center gap-8 pt-8 mt-auto">
        {/* Website Link */}
        {project.liveLink && (
          <a
          ...
          >
            ...
            <svg 
              className="w-3.5 h-3.5 ml-1.5 opacity-50 group-hover:opacity-100 transition-opacity" 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
